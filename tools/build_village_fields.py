...
class Scene:
    ...
    def ext_scene(self, path):
        """Reference another scene (a modular house) as an instanced ext_resource."""
        if path not in self.ext_ids:
            eid = "s_%s" % os.path.basename(path).replace(".tscn", "")
            self.ext_ids[path] = eid
            self.ext.append((path, eid))
...
def _basis_yaw(rot_y_deg):
    """Y-rotation basis in the order Godot's Transform3D text parser reads.

    The parser takes the nine numbers as a row-major matrix whose *columns* are
    the basis axes, so the text has to be
    `(X.x, Y.x, Z.x, X.y, Y.y, Z.y, X.z, Y.z, Z.z)`.
    `_basis()` below writes them as if each axis were a row, which mirrors every
    yaw it emits (rotation -angle). That is invisible on square props such as the
    plot pads, but a building placed with it lands rotated and offset, so
    anything whose facing matters uses this one instead.
    """
    r = math.radians(rot_y_deg)
    c, s = math.cos(r), math.sin(r)
    return "%.6f, 0, %.6f, 0, 1.000000, 0, %.6f, 0, %.6f" % (c, s, -s, c)
...
def emit_plots(scene):
    ...
    for name, x, z, y, facing, cluster, role in PLOTS:
        ...
        if True:  # every house gets cleared ground under it
...
# ---------------------------------------------------------------- houses
# The buildings themselves live as standalone scenes under scenes/houses/
# (generated by tools/build_house_scenes.gd) so the same cottage can be reused
# in any level. This pass only decides *which* cottage stands on each plot and
# which way it faces — the plot's rotation already aims a building at its lane.
HOUSE_VARIANTS = {
...
# Plot footprints in metres, from the house scenes (width along the lane x depth).
VAR_W = {"small": 2.0, "cottage": 3.0, "teal": 3.0}
VAR_D = {"small": 2.0, "cottage": 2.0, "teal": 2.0}


def _house_variant(name, cluster, role):
    """Deterministic per plot: a village street mixes its cottages, and the
    starter plot gets the plain clay-roofed one."""
    if role == "starter":
        return "cottage"
    roll = _h01(name, 71) + _h01(cluster, 73) * 0.5
    if roll < 0.34:
        return "teal"
    if roll < 0.60:
        return "small"
...
def emit_houses(scene):
    """Places a house scene on every plot. Each plot's rotation aims a building
    at its lane; the house scenes are built with their front on local +Z, while
    the plot faces its local +X down the road, so the placement adds the -90 turn
    and then centres the footprint on the marker."""
    scene.node("Houses", "", "Node3D")
    for name, x, z, y, facing, cluster, role in PLOTS:
        variant = _house_variant(name, cluster, role)
        path = HOUSE_VARIANTS[variant]
        eid = scene.ext_scene(path)
        # A plot's rotation aims its local +X at its lane; a house scene carries
        # its front on local +Z, so the placement adds +90 to put the front on
        # the road, and the house lands exactly on its marker.
        place_rot = facing + 90.0
        w, d = VAR_W[variant], VAR_D[variant]
        # The scene's origin is the footprint's -X/-Z corner, so the origin is
        # the marker minus the *rotated* half-extent: without that the house
        # walks off its plot by half its width (and straight into the pond).
        rad = math.radians(place_rot)
        c, s = math.cos(rad), math.sin(rad)
        ox = x - ((w * 0.5) * c + (d * 0.5) * s)
        oz = z + ((w * 0.5) * s - (d * 0.5) * c)
        scene.node(name, "Houses", None,
                   ["transform = Transform3D(%s, %s)" % (
                       _basis_yaw(place_rot), _fmt3((ox, PROP_Y + y, oz)))],
                   instance=eid)
...
def write_docs(plots):
    ...
    for p in sorted(plots, key=lambda r: r[0]):
        variant = _house_variant(p[0], p[5], p[6])
        rows.append("| `%s` | %.1f | %.1f | %.1f | %d\u00b0 | `%s` | %s | `%s` |" % (
            p[0], p[1], p[2], p[3], p[4], p[5], p[6], variant))
    table = "\n".join(rows)
    houses = "\n".join("| `%s` | %s |" % (k, v) for k, v in sorted(HOUSE_VARIANTS.items()))
    doc = DOC_TEMPLATE.replace("{{PLOTS}}", table).replace("{{COUNT}}", str(len(plots))).replace("{{HOUSES}}", houses)
    with open(path, "w") as f:
        f.write(doc)
    print("wrote %s" % path)
